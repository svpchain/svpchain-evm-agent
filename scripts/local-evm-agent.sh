...
capabilities="${EVM_AGENT_LOCAL_CAPABILITIES:-evm.defi,evm.relay}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --config-file) local_config_file="${2:-}"; shift 2 ;;
...
render_config() {
  ...
  fail "local config file not found: ${local_config_file}; copy scripts/local-evm-agent.toml.example first"
}
