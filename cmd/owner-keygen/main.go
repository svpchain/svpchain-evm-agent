// Command owner-keygen mints this agent's owner key: it writes a fresh
...
// bech32 with the chain's prefix, and the registration fee plus the bond have
// to be funded to that address before MsgRegisterAgent will succeed. So the
// same Go that parses the key mints it, and answers the only question that
// follows.
//
...
// tool cannot overwrite an existing identity. That refusal matters more here
// than it looks: this key is both the owner that holds the bond AND the
// operator the agent id derives from, so replacing one strands a registration
// and its bond with nothing on either side reporting a fault.
//
// scripts/deploy.sh --gen-owner-key is the intended caller; running it by hand
// is fine and takes the same one flag.
package main
...
func main() {
	...
	if err := run(*out); err != nil {
		fmt.Fprintf(os.Stderr, "owner-keygen: %v\n", err)
		os.Exit(1)
	}
}
